...
const { DEFAULT_RELAYS, isValidRelayUrl } = await import("./nostr-profile");
const { fetchNostrProfile, npubToHex, refreshNostrProfile } = await import("./nostr-profile.server");
...
describe("refreshNostrProfile", () => {
  ...
  it("uses DEFAULT_RELAYS when nostrRelays is null", async () => {
    //* Arrange
    mockQuerySync.mockResolvedValueOnce([{ content: JSON.stringify({ display_name: "Alice" }), created_at: 1000 }]);
