...
export const BetterAuthSignInForm: React.FC = () => {
  //* State
  const { t } = useTranslation();
  const form = useForm<SignInValues>({ resolver: standardSchemaResolver(signInSchema) });
  ...
  const onSubmit = async (data: SignInValues) => {
    ...
    // BA returns an opaque response — the two-factor plugin sets a dedicated
    // cookie when a second factor is required. Rely on the dedicated
    // /two-factor route to read that state from the server via getSession.
    const data2: Record<string, unknown> = (result.data ?? {}) as Record<string, unknown>;
    if (data2["twoFactorRedirect"] === true) {
      window.location.replace("/better-auth/two-factor");
      return;
    }
