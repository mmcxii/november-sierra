...
import { deleteWebhook, updateWebhook } from "@/lib/services/webhook";
...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  const result = await updateWebhook(auth.user, id, parsed.data);
...
export async function DELETE(request: Request, { params }: RouteParams) {
  ...
  const { id } = await params;
  const result = await deleteWebhook(auth.user, id);
  if (result.error != null) {
    return apiError(result.error.code, result.error.message, result.error.status);
  }
