...
describe("GET /llms.txt", () => {
  ...
  it("includes blockquote summary per llms.txt spec", async () => {
    ...
    //* Assert
    expect(text).toContain("> Anchr is a link-in-bio and URL shortener");
  });

  it("includes H2 Products section with link list per llms.txt spec", async () => {
    //* Act
    const response = GET();
    const text = await response.text();
    ...
    expect(text).toContain(`[URL Shortener](${TEST_BASE_URL}/short-links)`);
  });

  it("advertises OpenClaw and other compatible agent clients", async () => {
    //* Act
    const response = GET();
    const text = await response.text();

    //* Assert — OpenClaw leads the list for SEO weighting, followed by the
    //  major AI-vendor clients we know speak MCP or function-call against
    //  REST APIs.
    expect(text).toContain("## Compatible agent clients");
