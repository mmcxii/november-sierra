...
import { DEFAULT_RELAYS, type NostrProfileData } from "./nostr-profile";
...
function resolveDisplayName(content: Record<string, unknown>): null | string {
...
export async function fetchNostrProfile(npub: string, relays: string[]): Promise<null | NostrProfileData> {
...
export async function refreshNostrProfile(user: {
  id: string;
  nostrNpub: null | string;
  nostrRelays: null | string;
}): Promise<void> {
  if (user.nostrNpub == null) {
    return;
  }

  const relays: string[] = user.nostrRelays != null ? (JSON.parse(user.nostrRelays) as string[]) : DEFAULT_RELAYS;
