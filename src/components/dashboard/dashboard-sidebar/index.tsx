...
import { BarChart3, Link2, Menu, Settings, X } from "lucide-react";
    ...
    <>
      ...
      >
        ...
        <div className="flex h-14 items-center gap-3 px-4">
          <Link className="flex items-center gap-3" href="/dashboard" onClick={closeMobile}>
            <SiteLogo size="sm" />
            <span className="text-sidebar-foreground text-sm font-semibold tracking-wide uppercase">Anchr</span>
          </Link>
        </div>
        ...
        <div className="border-sidebar-border border-t p-3">
          <DropdownMenu>
            <DropdownMenuTrigger className="hover:bg-sidebar-accent flex w-full items-center gap-3 rounded-md px-3 py-2 outline-none">
              <Avatar size="sm">
                {user.avatarUrl && <AvatarImage alt={user.displayName ?? user.username} src={user.avatarUrl} />}
                <AvatarFallback>{initials}</AvatarFallback>
              </Avatar>
              <div className="flex flex-col items-start text-left">
                <span className="text-sidebar-foreground text-sm font-medium">{user.displayName ?? user.username}</span>
