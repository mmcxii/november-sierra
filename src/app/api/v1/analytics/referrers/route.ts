...
import { getReferrerAnalytics } from "@/lib/mcp/services/analytics";
...
export async function GET(request: Request) {
  ...
  const result = await getReferrerAnalytics(auth.user, { end: end?.toISOString(), start: start?.toISOString() });
